...
resource "sakuracloud_server" "isucon12q" {
  name = "isucon12q"
  zone = var.zone

  core   = 2
  memory = 4
  disks  = [sakuracloud_disk.isucon12q.id]
...
resource "sakuracloud_disk" "isucon12q" {
  name = "isucon12q"
  zone = var.zone
...
output "ip_address" {
  value = sakuracloud_server.isucon12q.ip_address
}